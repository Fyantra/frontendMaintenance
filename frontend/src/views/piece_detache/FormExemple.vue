...
const currentTimePlus = ref(
  `${String(now.getHours() + 1).padStart(2, "0")}:${String(now.getMinutes()).padStart(
    2,
    "0"
  )}`
);
...
const submitForm = async () => {
  ...
  if (!v$.value.$invalid) {
    ...
    if (dateFin < now && heureFin && heureFin < now) {
      form.identifiant_status_tache = 3; // "En retard"
    }

    formData.append("identifiant_status_tache", String(form.identifiant_status_tache));

    if (!isAllDay.value) {
      formData.append("heure_debut", String(form.heure_debut || ""));
      formData.append("heure_fin", String(form.heure_fin || ""));
    }
    formData.append(
      "temps_maintenance_heure",
      String(form.temps_maintenance_heure || "")
    );
    formData.append(
      "temps_maintenance_minute",
      String(form.temps_maintenance_minute || "")
    );
    formData.append("temps_arret_heure", String(form.temps_arret_heure || ""));
    formData.append("temps_arret_minute", String(form.temps_arret_minute || ""));
    ...
    if (tacheId) {
      ...
      //   router.push("/detailMachine/" + tacheId);
      // }
    } else {
      // Si c'est un ajout
...
<template>
  ...
  <div class="row mb-4">
    <div class="col-md-8">
      <h2 class="page-title">
        <!-- {{ tacheId ? "Modification de machine" : " Créer une nouvelle tâche" }} -->
    ...
    <div class="col-md-12">
      <div class="card shadow mb-4">
        <div class="card-header">
          <strong class="card-title"></strong>Ajouter une nouvelle tâche (* champ
          obligatoire)
        </div>
        <div class="card-body" ref="formError">
          <form @submit.prevent="submitForm">
            ...
            <button type="submit" class="btn mb-2 mt-4 btn-primary">
