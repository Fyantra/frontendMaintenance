...
const submitForm = async () => {
  ...
  if (!v$.value.$invalid) {
    ...
    if (fileInput.value?.files?.[0]) {    // Si il y a une nouvelle image
...
onMounted(async () => {
  ...
  await Promise.all([modeles.fetchItems(),ateliers.fetchItems(),fournisseurs.fetchItems(),]);
});
</script>
...
<template>
  ...
  <div class="row"><div class="col-md-8"><h2 class="page-title">{{ pieceId ? 'Modification de pièce détachée' : 'Créer une pièce détachée' }}</h2></div></div>

  <p>Élément d'un ensemble fonctionnel que l'on peut extraire, isoler ou remplacer.</p>

  <ErrorMessage v-if="errorMessage" :errorMessage="errorMessage" :error401Message="error401Message" :clearError="clearError"/>

  <div class="row">
    <div class="col-md-12">
      <div class="card shadow mb-4">
        <div class="card-header"><strong class="card-title"></strong>{{ pieceId ? 'Modifier une pièce détachée' : 'Ajouter une pièce détachée ' }} (* champ obligatoire)</div>
        <div class="card-body">
          <form @submit.prevent="submitForm">
            <div class="d-flex justify-content-center">
              <div class="card shadow mb-4">
                ...
                <div class="card-body">
                  <span v-if="image" class="circle circle-sm bg-danger justify-content-center" @click="removeImage" style="float: right;">
                    <i class="material-icons notranslate text-white">delete</i>
                  </span>
                  <div id="drag-drop-area" class="upload-frame" @dragover.prevent @click="triggerFileInput">
                    <div v-if="!image" class="default-message text-center"><i class="material-icons notranslate">handyman</i><p>Glisser ou deposer votre image ici</p></div>
                    <div v-else class="uploaded-image"><img :src="image" alt="Pièce détachée" class="img-fluid" /></div>
                  </div>
                  <input type="file" ref="fileInput" accept="image/*" @change="handleFileUpload" class="d-none"/>
                </div>
              </div>
            </div>
            ...
            <div class="form-row">
              <div class="form-group col-md-6">
                ...
                <input type="text" class="form-control" id="nom" placeholder="Nom de la pièce détachée"
                  v-model="form.nom_piecedetache" :class="{'is-invalid':v$.nom_piecedetache.$invalid && v$.nom_piecedetache.$dirty,}"/>
                <span v-if="v$.nom_piecedetache.$error" class="error">Nom pièce détachée requis.</span>
              </div>
              <div class="form-group col-md-6">
                <label for="modele">Modele</label>
                <select id="modele" class="form-control" v-model="form.modele_id">
                  <option selected disabled>Selectionner un modele</option>
                  <option v-for="modele in modeles.items.value" :key="modele.id" :value="modele.id">
                    {{ modele.nom_modele }}
                  </option>
                </select>
              </div>
            ...
            <div class="form-row">
              <div class="form-group col-md-6">
                ...
                <div class="input-group">
                  ...
                  <input type="number" class="form-control" id="prix" placeholder="10 000"
                    v-model="form.prix_unitaire" :class="{'is-invalid': v$.prix_unitaire.$invalid && v$.prix_unitaire.$dirty,}"/>
                </div>
                <span v-for="error of v$.prix_unitaire.$errors" :key="error.$uid" class="error">{{ error.$message }}</span>
              </div>
              <div class="form-group col-md-6">
                <label for="quantite">Quantite*</label>
                <div class="input-group">
                  <input type="number" class="form-control" id="quantite" placeholder="1"
                    v-model="form.quantite" :class="{'is-invalid': v$.quantite.$invalid && v$.quantite.$dirty,}"
                  />
                </div>
                <span v-for="error of v$.quantite.$errors" :key="error.$uid" class="error">{{ error.$message }}</span>
              </div>
            </div>
            ...
            <div class="form-row">
              <div class="form-group col-md-6">
                ...
                <div class="input-group">
                  <div class="input-group-prepend">
                    <span id="form-icon" class="input-group-text material-icons notranslate">expand_less</span>
                  </div>
                  <input type="number" class="form-control" id="stock_min" placeholder="Seuil minimum pour prevoir réapprovisionnement"
                    v-model="form.stock_min" :class="{'is-invalid': v$.stock_min.$invalid && v$.stock_min.$dirty,}"
                  />
                </div>
                <span v-for="error of v$.stock_min.$errors" :key="error.$uid" class="error">{{ error.$message }}</span>
              </div>
              ...
              <div class="form-group col-md-6">
                ...
                <div class="input-group">
                  <div class="input-group-prepend">
                    <span id="form-icon" class="input-group-text material-icons notranslate" >expand_more</span>
                  </div>
                  <input type="number" class="form-control" id="stock_max" placeholder="Seuil maximum a ne pas depasser pour réapprovisionnement"
                    v-model="form.stock_max" :class="{'is-invalid': v$.stock_max.$invalid && v$.stock_max.$dirty,}"
                  />
                </div>
                <span v-for="error of v$.stock_max.$errors" :key="error.$uid" class="error">{{ error.$message }}</span>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="input-group">
                <div class="input-group-prepend"><span id="form-icon" class="input-group-text material-icons notranslate">shopping_cart</span></div>
                <input type="number" class="form-control" id="lot" placeholder="Quantite minimum de réapprovisionnement"
                  v-model="form.lot_de_reapprovisionnement"
                  :class="{'is-invalid': v$.lot_de_reapprovisionnement.$invalid && v$.lot_de_reapprovisionnement.$dirty,}"
                />
              </div>
              <span v-for="error of v$.lot_de_reapprovisionnement.$errors" :key="error.$uid" class="error">{{ error.$message }}</span>
            </div>
            ...
            <div class="form-group">
              ...
              <select id="emplacement" class="form-control" v-model="form.emplacement_id">
                ...
                <option v-for="atelier in ateliers.items.value" :key="atelier.id" :value="atelier.id">
                  {{ atelier.nom_atelier }}
                </option>
              </select>
            </div>
            ...
            <div class="form-group">
              ...
              <input class="form-control" id="date_achat" type="datetime-local" v-model="form.date_achat">
            </div>

            <div class="form-row">
              <div class="form-group col-md-6">
                <label for="reference">Référence fabricant</label>
                <input type="text" class="form-control" id="reference" v-model="form.reference_fabricant"/>
              </div>
              <div class="form-group col-md-6">
                <label for="fournisseur">Fournisseur</label>
                <select id="fournisseur5" class="form-control" v-model="form.fournisseur_id">
                  <option selected disabled>Selectionner un fournisseur</option>
                  <option v-for="fournisseur in fournisseurs.items.value" :key="fournisseur.id" :value="fournisseur.id">
